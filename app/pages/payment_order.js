...
class PaymentOrder extends BasePage {
  constructor(){
    super();
    this.state = {
      password:'',
...
onRight_ButtonClick(){
  ...
  if(this.state.index == 1){
    this.refs.dialog.close()
       ...
       if(res_json.retCode==1){
         AresAPI.OrderController.paySuccess({orderNo:this.props.orderNo}).done((res_data,res)=>{
  ...
  //      this.setState({phone:''})
        ...
        this.props.navigator.push({id:'ShoppingSuccess',
        params:{acctno:'二类账户',orderNo:this.props.orderNo,phone:this.state.phone,amt:this.props.price+10}})
         }else{
  ...
  //      this.setState({phone:''})
        ...
        this.props.navigator.push({id:'ShoppingSuccess',params:{acctno:'三类账户',orderNo:this.props.orderNo,phone:this.state.phone,amt:this.props.price+10}})
         }else{
  ...
  if(this.state.isShow==true){
    return(
       <View style={styles.root}>
         ...
         <View style={{borderColor:'#e5e5e5',marginTop:10,height:157,backgroundColor:'#fff'}}>
           <View style={{marginTop:11,flexDirection:'row',backgroundColor:'#fff',justifyContent:'space-between'}}>
             ...
             <Text style={{fontSize:16,color:"#f29836",marginRight:13}}>￥{this.props.price.toFixed(2)}</Text>
           </View>
