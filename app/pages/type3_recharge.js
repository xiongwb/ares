...
class Type3Recharge extends BasePage  {
   constructor (props) {
    super(props);
    this.state = {
  ...
  onRight_ButtonClick(){
    this.refs.dialog.close()
        AresAPI.TranAcctSelf.tranAcctSelf1To3({
        phone:this.state.phone,
        custno:this.state.custno,
        acctno3:this.props.account3,
       ...
       if(res_json.retCode==1){
        ...
        this.props.navigator.push({id:'WithdrawalsSuccess',params:{data:'充值'}})
         }else{
 ...
 on_Click(){
   ...
   if (regexp.test(this.state.bal) === false) {
     Alert.alert('提示', '对不起，金额精确到两位小数的纯数字', [{ text: '确定'}]);
     return;
   }
   if(this.state.bal>1000){
     Alert.alert('提示', '对不起，三类账户为零钱包，不支持1000元以上交易', [{ text: '确定'}]);
     return
   }
       ...
       <View style={styles.root}>
        <BackNavBar component={this}>充值</BackNavBar>
          <View style={{alignItems:'center',marginTop:12}} >
            <Text style={{ fontSize:12,color:'#D3D3D3'}}>银行卡转入到Ⅲ类账户</Text>
          </View>
          <View style={{marginTop:12,marginBottom:33}}>
            <AresTextInput editable={false} placeholderTextColor="#101010" placeholder={this.state.account} title="银行" borderTopWidth={0.5} borderBottomWidth={0.5}  />
          </View>
            <AresTextInput maxLength={6} borderWidth={0.5} title="金额" onChangeText={this.on_change.bind(this)} />
          <View style={styles.button_box}>
            <Button type="primary"
             disabled={!this.state.sign_up_params_validate}
             onClick={this.on_Click.bind(this)}>确认充值</Button>
          </View>
          <Dialog
            ref="dialog"
            title="请输入交易密码"
...
export default createForm()(Type3Recharge)
