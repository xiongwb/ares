...
class ResetLoginPwd1 extends BasePage {
  constructor(props) {
    super(props);
    this.state={
      disabled:true,
  ...
  componentDidMount(){
  //let  phone:this.props.user_info.tokenVO!=null?this.props.user_info.tokenVO.phone:null,
  //  console.log(phone);
    if(this.props.user_info){
      AresAPI.ResetLoginPwd.getVarCode({
        brc:this.props.user_info.brcNo||'1',
        phone:this.props.user_info.phone!=null?this.props.user_info.phone:null,
  ...
  sendVarCode(){
    if(this.props.user_info){
      AresAPI.ResetLoginPwd.getVarCode({
        ...
        if(res_json.retCode != 1){
          Alert.alert('错误提示', res_json.retMsg, [{ text: '确定'}])
          //console.log(phone:this.props.user_info.tokenVO!=null?this.props.user_info.tokenVO.phone:null);
        }
      })
    }else{
        Alert.alert('错误提示', '尚未登录', [{ text: '确定'}])
  ...
  submit(){
    ...
    if(this.props.user_info){
      AresAPI.ResetLoginPwd.validateVarCode({
        ...
        if(res_json.retCode == 1){
          this.props.navigator.push({id:'ResetLoginPwd2',params:{user_info:this.props.user_info}})
        }else{
          Alert.alert('错误提示', res_json.retMsg, [{ text: '确定'}])
        }
      })
