...
@property (nonatomic, strong) CALayer * scanStatics;
...
- (CAShapeLayer *)scanRectLayer
{
    if (!_scanRectLayer) {
        ...
        _scanRectLayer.strokeColor = [UIColor blueColor].CGColor;
    }
...
- (CALayer *)scanStatics
{
  if (!_scanStatics) {
    CGRect scanRect = self.scanRect;
    scanRect.origin.x -= 1;
    scanRect.origin.y = scanRect.origin.y - self.scanRect.size.height - 1;
    scanRect.size.width += 2;
    scanRect.size.height += 2;

    _scanStatics = [CALayer layer];
    _scanStatics.frame = scanRect;
    _scanStatics.contents = (__bridge id _Nullable)([UIImage imageNamed:@"qrcode_statics.png"].CGImage);
    _scanStatics.opacity = 0.0f;
...
- (void)runScanStatics
{
    CGPoint toPosition = CGPointMake(self.scanStatics.position.x, self.scanStatics.position.y + self.scanRect.size.height);
    ...
    anima.fromValue=[NSValue valueWithCGPoint:self.scanStatics.position];
    anima.toValue=[NSValue valueWithCGPoint:toPosition];
  
    CAAnimationGroup *groupAnima = [CAAnimationGroup animation];
    groupAnima.animations = @[fade, anima];
   //设置组动画的时间
    groupAnima.duration = 3;
    groupAnima.repeatCount = HUGE_VALF;
    groupAnima.removedOnCompletion = NO;
    groupAnima.timingFunction = [CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionEaseIn];
    [self.scanStatics addAnimation:groupAnima forKey:nil];
...
- (void)setupScanRect
{
    ...
    [self.layer addSublayer: self.scanStatics];
}
